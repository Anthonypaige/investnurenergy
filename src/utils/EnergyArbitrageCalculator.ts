document.addEventListener('DOMContentLoaded', function () {
  ...
  function calculateSavings() {
    const batteryCapacity: number =
      parseFloat((document.getElementById('batteryCapacity') as HTMLInputElement).value) || 0;
    ...
    const summerPeakHours: number =
      parseFloat((document.getElementById('summerPeakHours') as HTMLInputElement).value) || 0;
    const nonSummerPeakHours: number =
      parseFloat((document.getElementById('nonSummerPeakHours') as HTMLInputElement).value) || 0;
    const averageConsumption: number =
      parseFloat((document.getElementById('averageConsumption') as HTMLInputElement).value) || 0;
    ...
    (document.getElementById('results') as HTMLElement).innerHTML =
      'Total Savings Per Year: $' + totalSavings.toFixed(2);
  }
});
